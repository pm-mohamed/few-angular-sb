...
> Keywords: Storybook - SCSS - Typography Classification
...
- Use [Coolors.co](https://coolors.co/) to generate new color variations.
...
- Create new file `secondary.stories.mdx` and define secondary button variation just like the primary button, define the new hover color then add it to the colors Swatches then create the mdx file, pass it the correct props and style it inside `pm-button.component.scss` (the type and the html is already added).
...
- import Meta and Description then use the correct meta and description then write a title for this story.
